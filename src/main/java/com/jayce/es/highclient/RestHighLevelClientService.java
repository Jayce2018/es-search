...
public class RestHighLevelClientService {
     ...
     * @throws IOException
     */
    public BulkResponse importAll(String indexName, boolean isAutoId, List<Cloth> source) throws IOException {
        if (0 == source.size()) {
            //todo 抛出异常 导入数据为空
            return null;
        }
        ...
        if (isAutoId) {
            ...
            for (Cloth s : source) {
                request.add(new IndexRequest(indexName).id(JSONObject.parseObject(s.toString()).getString("id")).source(JSONObject.toJSONString(s), XContentType.JSON));
            }
        }
